...
class Healer(Hero):
    ...
    def healing(self, friend):
        friend.set_hp(int(friend.get_hp() + self.magic))
        print(f'== Исцеляю друга по имени {friend.name}! Теперь у него {friend.get_hp()} HP! ==')

    def make_a_move(self, friends, enemies):
        print(self.name, end=' ')
        if random.randint(1, 100) % 100 == 0 and len(enemies) != 0:
            self.attack(random.choice(enemies))
        elif random.randint(1, 100) % 3 == 0:
            self.healing(friends[0])
        elif random.randint(1, 100) % 2 == 0:
            self.healing(friends[random.randint(1, 2)])
        else:
            self.healing(random.choice(friends))
        super().make_a_move(friends, enemies)
        print('\n')

    def __str__(self):
        return 'Имя: {0} | HP: {1} | Класс: Хилер | Магическая сила: {2} | Энергия: {3}'.format(
            self.name,
            round(self.get_hp(), 1),
            round(self.magic, 1),
            round(self.get_power(), 1)
        )
...
class Tank(Hero):
    ...
    def make_a_move(self, friends, enemies):
        print(self.name, end=' ')
        if random.randint(1, 100) % 3 != 0 and len(enemies) != 0:
            self.attack(random.choice(enemies))
        else:
            self.change_shield()
        super().make_a_move(friends, enemies)
    ...
    def __str__(self):
        return 'Имя: {0} | HP: {1} | Класс: Танк | Щит: {2} | Показатель защиты: {3} | Энергия: {4}'.format(
            self.name,
            round(self.get_hp(), 1),
            self.shield,
...
class Attacker(Hero):
    ...
    def make_a_move(self, friends, enemies):
        print(self.name, end=' ')
        if len(enemies) != 0 and self.power_multiply >= 10:
    ...
    def __str__(self):
        return 'Имя: {0} | HP: {1} | Класс: Убийца | Показатель усиления: {2} | Энергия: {3}'.format(
            self.name,
            round(self.get_hp(), 1),
            round(self.power_multiply, 1),
            round(self.get_power(), 1)
        )
